...
class Web3Signature(BaseModel):
    ...
    expire_at: Optional[StrictInt] = None
    __properties: ClassVar[List[str]] = ["status", "strategy", "nonce", "attempts", "expire_at"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['unverified', 'verified', 'failed', 'expired']):
            raise ValueError("must be one of enum values ('unverified', 'verified', 'failed', 'expired')")
        return value

    @field_validator('strategy')
    def strategy_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['web3_metamask_signature', 'web3_coinbase_signature']):
            raise ValueError("must be one of enum values ('web3_metamask_signature', 'web3_coinbase_signature')")
        return value
