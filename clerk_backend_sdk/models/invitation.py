...
class Invitation(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["object", "id", "email_address", "public_metadata", "revoked", "status", "url", "created_at", "updated_at"]

    @field_validator('object')
    def object_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['invitation']):
            raise ValueError("must be one of enum values ('invitation')")
        return value

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['pending', 'accepted', 'revoked']):
            raise ValueError("must be one of enum values ('pending', 'accepted', 'revoked')")
        return value
