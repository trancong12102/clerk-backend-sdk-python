...
class Passkey(BaseModel):
    ...
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['verified']):
            raise ValueError("must be one of enum values ('verified')")
        return value
    ...
    def nonce_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['nonce']):
            raise ValueError("must be one of enum values ('nonce')")
        return value
